...
interp_result1 <- with(combinations_rearranged, 
                       akima::interp(x = coef_x1, y = coef_x2, z = auc, duplicate = "mean"))

# Interpolate auc values for contour plot using coef_x2 and intercept
interp_result2 <- with(combinations_rearranged, 
                       akima::interp(x = coef_x1, y = coef_x3, z = auc, duplicate = "mean"))

# Interpolate auc values for contour plot using coef_x3 and intercept
interp_result3 <- with(combinations_rearranged, 
                       akima::interp(x = coef_x2, y = coef_x3, z = auc, duplicate = "mean"))
...
fig_contour1 <- plot_ly(x = interp_result1$x, 
                        y = interp_result1$y, 
                        z = interp_result1$z, 
                        type = 'contour', 
                        colorscale = custom_colorscale,   # Use the custom color scale
                        zmin = 0.45,                       # Set minimum AUC value
                        zmax = max(interp_result1$z, na.rm = TRUE)) %>%
  layout(title = 'Coefficient of x1, Coefficient of x2, and AUC',
         xaxis = list(title = 'Coefficient β₁' , range = x_contour_range),         # Label for x-axis
...
fig_contour2 <- plot_ly(x = interp_result2$x, 
  ...
  layout(title = 'Coefficent of x1, Coefficient of x3, and AUC',
         xaxis = list(title = 'Coefficient β₁' , range = x_contour_range),         # Label for x-axis
         yaxis = list(title = 'Coefficient β₃' , range = y_contour_range)) %>%
  colorbar(title = 'AUC')  # Title for the color legend as "AUC"
...
fig_contour3 <- plot_ly(x = interp_result3$x, 
  ...
  layout(title = 'Coefficient of x2, Coefficient of x3, and AUC',
         xaxis = list(title = 'Coeffiecient β₂' , range = x_contour_range),         # Label for x-axis
         yaxis = list(title = 'Coefficient β₃' , range = y_contour_range)) %>%
  colorbar(title = 'AUC')  # Title for the color legend as "AUC"
...
fig_contour_prev1 <- plot_ly(x = interp_results_prev1$y, 
  ...
  layout(title = 'Coefficient of x1, Coefficient of x2, and Prevalence',
         xaxis = list(title = 'Coefficient β₁', range = x_contour_range_prev),         # Label for x-axis
...
fig_contour_prev2 <- plot_ly(x = interp_results_prev2$y, 
                             ...
                             type = 'contour', 
                             colors = colorRamp(c("yellow", "red"))) %>%
  layout(title = 'Coefficient of x1, Coefficient of x3, and Prevalence',
         xaxis = list(title = 'Coeffiecient β₁', range = x_contour_range_prev),         # Label for x-axis
         yaxis = list(title = 'Coefficient β₃', range = y_contour_range_prev)) %>%
  colorbar(title = 'Prevalence')  # Title for the color legend as "AUC"
...
fig_contour_prev3 <- plot_ly(x = interp_results_prev3$y, 
                             ...
                             type = 'contour', 
                             colors = colorRamp(c("yellow", "red"))) %>%
  layout(title = 'Coefficient of x2, Coefficient of x3, and Prevalence',
         xaxis = list(title = 'Coefficient β₂', range = x_contour_range_prev),         # Label for x-axis
         yaxis = list(title = 'Coefficient β₃', range = y_contour_range_prev)) %>%
  colorbar(title = 'Prevalence')  # Title for the color legend as "AUC"
